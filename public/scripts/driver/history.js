...
function app() {
    ...
    var myViewModel = function () {
        ...
        this.displayCategory = function (clickedCategory) {
            ...
            firebase.database().ref("requests/" + clickedCategory.name()).orderByChild('driver_id').equalTo(currentUser.uid).on("value", function (request) {
                if (!request.val()) {
    ...
    function getRequests(currentCategory) {
        ...
        firebase.database().ref("requests/" + currentCategory.name()).orderByChild('driver_id').equalTo(currentUser.uid).on("child_added", function (request) {
            currentCategory.list.push(new Request(request.key, request.val().id, request.val().state, request.val().date, request.val().time));
        });

        firebase.database().ref("requests/" + currentCategory.name()).orderByChild('driver_id').equalTo(currentUser.uid).on("child_removed", function (removedRequest) {
